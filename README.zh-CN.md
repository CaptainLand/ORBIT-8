...
> `ORBIT-8-v2.2-models.zip`，并将其中两个 `.pt` 文件放入 `v22/releases/`。
> 训练数据不会随项目分发。
...
| ORBIT-8 v2.2 HandFlow | 留出集校准的 16M Transformer | 计划采样 Trans 排键模型 | 全曲 HandFlow 验收 |

当前实验主线为 **ORBIT-8 v2.2 HandFlow**。它保留 1600 万参数节奏骨干，并使用留出集进行
起音校准；排键模型加入计划采样，使训练过程更接近实际生成。发布版排键模型约有 300 万参数，
全曲验收会继续检查双手占用、长物件、星星轨迹与尾部，以及不规则换手。
...
v2.2 预览版目前需要 Windows、Python 3.12，以及驱动支持 CUDA 12.8 的 NVIDIA 显卡。

1. 克隆本仓库，或下载并解压源码包。
2. 前往 [Releases](https://github.com/CaptainLand/ORBIT-8/releases) 下载
   `ORBIT-8-v2.2-models.zip`，将 `orbit_v22_rhythm.pt` 和
   `orbit_v22_arranger.pt` 放入 `v22/releases/`。
3. 在项目目录打开 PowerShell，安装运行环境：
...
对外分发。发行包只包含运行 ORBIT-8 v2.2 所需的推理权重、评估报告与精简运行时元数据。
